...
options["seq_len"] = 64
options["max_len"] = 64  # p99 logs/window=53 at 300s; 64 covers all sequences
options["min_len"] = 2   # median window has 3 logs; 5 would drop >50% of sequences
...
print(f"device {options['device']} | logkey={options['is_logkey']} time={options['is_time']} "
      f"mask_ratio={options['mask_ratio']}")
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        print(f"vocab size: {len(vocab)} -> {options['vocab_path']}")
