...
def batch_level_evaluation(model, pooler, device, method, dataset, n_dim=0, e_dim=0):
    ...
    y = np.array(y_list)
    if 'knn' not in method:
        raise NotImplementedError
    if dataset == 'soar':
        return evaluate_soar_using_knn(x, y, data['splits'])
    return evaluate_batch_level_using_knn(100, dataset, x, y)


def evaluate_soar_using_knn(embeddings, labels, splits):
    x, y = embeddings, labels
    train_mask = np.array([s == 'train' and l == 0 for s, l in zip(splits, y)])
    test_mask = np.array([s == 'test' for s in splits])
    x_train, x_test = x[train_mask], x[test_mask]
    y_test = y[test_mask]

    n_neighbors = min(int(x_train.shape[0] * 0.02), 10)
    n_neighbors = max(n_neighbors, 1)

    mu, sd = x_train.mean(0), x_train.std(0)
    x_train = (x_train - mu) / (sd + 1e-6)
    x_test = (x_test - mu) / (sd + 1e-6)

    nbrs = NearestNeighbors(n_neighbors=n_neighbors)
    nbrs.fit(x_train)
    distances, _ = nbrs.kneighbors(x_train, n_neighbors=n_neighbors)
    mean_distance = distances.mean() * n_neighbors / max(n_neighbors - 1, 1)
    ...
    best_thres = threshold[best_idx] if best_idx < len(threshold) else threshold[-1]
